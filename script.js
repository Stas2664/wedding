...
// Ensure runs after DOM is ready
window.addEventListener('DOMContentLoaded', function(){
  ...
  }catch(err){ console.error('Lightbox init error:', err); }
});

// Lightbox (bulletproof, self-injecting)
(function(){
  ...
  function collectAnchors(){
    const selector = '.portfolio-image > a, .portfolio-items a, a[data-lightbox="portfolio"]';
    const list = Array.from(document.querySelectorAll(selector)).filter(a => /\.(jpe?g|png|webp|gif)$/i.test(a.getAttribute('href')||''));
    return list;
  }
  ...
  function init(){
    ...
    document.body.addEventListener('click', function(e){
      const a = e.target.closest('.portfolio-image > a, .portfolio-items a, a[data-lightbox="portfolio"]');
      if (!a) return;
      e.preventDefault();
      const items = collectAnchors();
      const href = a.getAttribute('href');
      const i = items.findIndex(x => x.getAttribute('href') === href);
      openAt(i === -1 ? 0 : i);
    });
